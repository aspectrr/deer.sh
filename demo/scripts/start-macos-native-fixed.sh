...
# Uses awk to normalize MACs from ARP table (macOS collapses zeros)
discover_ip_by_mac() {
    local mac="$1"
    local timeout="${2:-300}"
    local ip=""
    ...
    log "Discovering IP for MAC ${mac} (up to ${timeout}s)..."
    for _ in $(seq 1 "$timeout"); do
        ip=$(arp -an 2>/dev/null | awk -v target="$mac_normalized" '
        {
            # Extract MAC and IP from line like: "? (192.168.105.58) at 52:54:0:4f:fb:3c on bridge100"
            ip = $2
            mac = $4

            # Normalize MAC: remove colons, lowercase
            gsub(/:/, "", mac)
            tolower(mac)

            # Pad single hex digits with leading zero
            padded = ""
            n = split(mac, hex, "")
            for (i = 1; i <= length(hex); i++) {
                if (length(hex[i]) == 1) padded = padded "0" hex[i]
                else padded = padded hex[i]
            }

            # Reconstruct with colons
            formatted = padded
            n = split(padded, chars, "")
            for (i = 1; i <= length(chars); i++) {
                if (i < length(chars)) formatted = formatted chars[i] ":"
            }

            if (formatted == target || tolower(formatted) == target) {
                print ip
            }
        }' | head -1)
...
scp_to_source() {
    ...
    local src="$2"
    local dst="$3"
    scp -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o BatchMode=yes \
...
# HVF context must stay in same process - fork via -daemonize is not used.
boot_qemu_vm() {
    local overlay="$1"
...
if [ "$DRY_RUN" -eq 0 ]; then
    ...
    [ -x "$SOCKET_VMNET_CLIENT" ] || fail "socket_vmnet_client not found at $SOCKET_VMNET_CLIENT. Run: brew install socket_vmnet && sudo brew services start socket_vmnet"
...
UBUNTU_IMAGE="${DEER_IMAGES_DIR}/ubuntu-${UBUNTU_VERSION}-${UBUNTU_ARCH}.qcow2"

log "Downloading arm64 microVM assets..."
...
if [ "$DRY_RUN" -eq 0 ] && [ -f "$SOURCE_VM_PID_FILE" ]; then
...
# Fallback: kill any QEMU processes for deer-source
pkill -f "qemu-system-aarch64.*deer-source" 2>/dev/null || true

# Generate or reuse MAC address (persistent across restarts for ARP stability)
if [ "$DRY_RUN" -eq 0 ]; then
    if [ ! -f "$SOURCE_VM_MAC_FILE" ]; then
...
runcmd:
  ...
  - mkdir -p /etc/logstash/conf.d /var/log/logstash /usr/share/logstash/data
  - chown -R logstash:logstash /var/log/logstash /usr/share/logstash/data 2>/dev/null || true
  - printf '[Service]\nEnvironment=\"LS_JAVA_OPTS=-Xms512m -Xmx512m\"\n' > /etc/systemd/system/logstash.service.d/override.conf
  - sed -i 's/-Xms1g/-Xms512m/g;s/-Xmx1g/-Xmx512m/g' /etc/logstash/jvm.options 2>/dev/null || true
  - systemctl daemon-reload
  - systemctl enable logstash
"

SOURCE_META_DATA="instance-id: deer-source-001
...
SOURCE_IP=""
if [ "$DRY_RUN" -eq 0 ]; then
    SOURCE_IP="$(discover_ip_by_mac "$SOURCE_VM_MAC" 300)" || fail "Could not discover source VM IP. Check ${SOURCE_VM_SERIAL_LOG}"
    log "Source VM IP: ${SOURCE_IP}"

    wait_for_ssh "$SOURCE_IP" "$SOURCE_VM_SSH_USER" "$SOURCE_KEY" 300 || \
        fail "SSH to source VM timed out. Check ${SOURCE_VM_SERIAL_LOG}"

    log "Waiting for cloud-init to finish installing Logstash (may take several minutes)..."
    wait_for_cloud_init "$SOURCE_IP" "$SOURCE_VM_SSH_USER" "$SOURCE_KEY" 900 || \
        fail "cloud-init timed out on source VM. Check ${SOURCE_VM_SERIAL_LOG}"
    ...
    ssh_source "$SOURCE_IP" "sudo systemctl restart logstash"
