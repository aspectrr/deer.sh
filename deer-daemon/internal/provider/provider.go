...
func (r CreateRequest) WantsKafkaBroker() bool {
	if r.KafkaBroker != nil {
		return true
	}
	for _, ds := range r.DataSources {
		if ds.Type == DataSourceTypeKafka {
			return true
		}
	}
	return false
...
func NormalizeCreateRequestResources(req CreateRequest, defaultVCPUs, defaultMemoryMB int) (CreateRequest, bool) {
	...
	if defaultMemoryMB <= 0 {
		defaultMemoryMB = DefaultSandboxMemMB
	}

	if req.VCPUs <= 0 {
		req.VCPUs = defaultVCPUs
	}
	if req.MemoryMB <= 0 {
	...
	clamped := false
	if req.WantsKafkaBroker() {
		if req.VCPUs < KafkaBrokerMinVCPUs {
			req.VCPUs = KafkaBrokerMinVCPUs
			clamped = true
		}
		if req.MemoryMB < KafkaBrokerMinMemoryMB {
			req.MemoryMB = KafkaBrokerMinMemoryMB
			clamped = true
		}
	}
...
// KafkaBrokerConfig requests a sandbox-local Kafka-compatible broker.
