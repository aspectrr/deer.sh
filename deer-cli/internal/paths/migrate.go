...
// MaybeMigrate checks for the legacy ~/.deer directory and copies files
// to the new XDG locations if migration has not already completed.
// It does NOT delete ~/.deer - the user can do that manually.
func MaybeMigrate() error {
	...
	oldDir := filepath.Join(home, ".deer")
	...
	fmt.Fprintf(os.Stderr, "You can safely remove ~/.deer after verifying the migration.\n")
	return nil
}
