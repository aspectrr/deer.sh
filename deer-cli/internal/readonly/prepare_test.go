...
func TestPrepare_CommandContent(t *testing.T) {
	...
	if !strings.Contains(decoded[4], "/etc/ssh/deer_ca.pub") {
		t.Error("step 5 should write to /etc/ssh/deer_ca.pub")
	}
...
func TestDeployDaemonKey_Success(t *testing.T) {
	...
	// The command should target deer-daemon user's authorized_keys
	cmd := commands[0]
	...
	if !strings.Contains(decoded, "~deer-daemon/.ssh/authorized_keys") {
		t.Error("command should target ~deer-daemon/.ssh/authorized_keys")
	}
	...
	if !strings.Contains(decoded, "chown -R deer-daemon:deer-daemon") {
		t.Error("command should set ownership to deer-daemon")
	}
}
...
func TestSetupSourceHost_Success(t *testing.T) {
	...
	// Should create deer-daemon user
	if !strings.Contains(decoded, "useradd --system --shell /bin/bash -m deer-daemon") {
		t.Error("command should create deer-daemon user")
	}
	// Should be idempotent (check if user exists first)
	if !strings.Contains(decoded, "id deer-daemon") {
		t.Error("command should check if deer-daemon user exists")
	}
	// Should add to libvirt group
	if !strings.Contains(decoded, "usermod -aG libvirt deer-daemon") {
		t.Error("command should add deer-daemon to libvirt group")
	}
	// Should deploy key to deer-daemon's authorized_keys
	if !strings.Contains(decoded, "~deer-daemon/.ssh/authorized_keys") {
		t.Error("command should target ~deer-daemon/.ssh/authorized_keys")
	}
	if !strings.Contains(decoded, key) {
	...
	if !strings.Contains(decoded, "sudoers.d/deer-daemon-qemuimg") {
