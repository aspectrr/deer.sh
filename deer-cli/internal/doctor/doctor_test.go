...
func TestRunAllAllPass(t *testing.T) {
	run := func(ctx context.Context, command string) (string, string, int, error) {
		// Everything succeeds
		if strings.Contains(command, "systemctl is-active deer-daemon") {
			return "active\n", "", 0, nil
		}
		if strings.Contains(command, "systemctl is-enabled deer-daemon") {
			return "enabled\n", "", 0, nil
		}
		if strings.Contains(command, "systemctl is-active libvirtd") {
			return "active\n", "", 0, nil
		}
		if strings.Contains(command, "which deer-daemon") {
			return "/usr/local/bin/deer-daemon\n", "", 0, nil
		}
		...
		if strings.Contains(command, "test -d /var/lib/deer") {
			return "", "", 0, nil
		}
