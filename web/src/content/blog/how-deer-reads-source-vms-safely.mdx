...
<div class="ro-container">
  ...
  <div class="ro-input-bar">
    <span class="ro-prompt">$</span> Agent sends command:{' '}
    <span class="ro-cmd">cat /etc/nginx/nginx.conf</span>
  </div>
  ...
  <div class="ro-layer">
    <div class="ro-layer-header">
      ...
      <div>
        ...
        <div class="ro-tech">Go, in deer CLI</div>
      </div>
    </div>
  ...
  <div class="ro-layer">
    ...
    <ul class="ro-layer-details">
      <li>Certificate issued with principal "deer-readonly"</li>
      <li>VM's sshd maps this principal to the deer-readonly user</li>
...
Before any SSH connection is established, the deer CLI validates the command in Go. The validator lives in `deer/internal/readonly/validate.go` and follows a strict allowlist approach -- not a blocklist.
...
Validation happens at the top of `RunSourceVMCommand` in `deer/internal/vm/service.go`, before IP discovery or credential generation:
...
<BlogInlineCTA feature="This is how Fluid validates every command before it touches your servers - try it on your own infrastructure." />
...
| Source VM (read-only) | `deer-readonly` | `deer-readonly` | `/usr/local/bin/deer-readonly-shell` |

When the deer CLI requests credentials for a source VM, the key manager issues a certificate with the `deer-readonly` principal:
...
During `deer source prepare`, the source VM's `sshd_config` is configured with:
...
The file `/etc/ssh/authorized_principals/deer-readonly` contains exactly one line: `deer-readonly`. This means sshd will only accept certificates with the `deer-readonly` principal when authenticating as the `deer-readonly` user. A certificate with the `sandbox` principal cannot log in as `deer-readonly`, and vice versa.

The `deer-readonly` user is created as a system user with `/usr/local/bin/deer-readonly-shell` as its login shell. There is no password. There is no home directory. The only way in is via a valid certificate from the CA, and the only thing that runs is the restricted shell.
...
The final layer runs on the VM itself. Even if a command passes client-side validation and arrives via a valid certificate, the restricted shell at `/usr/local/bin/deer-readonly-shell` performs its own independent validation.
...
When the restricted shell blocks a command, it exits with code **126** -- the conventional Unix code for "command not executable." The deer CLI recognizes this exit code and logs it as a server-side block, distinct from a client-side rejection.
...
<div class="exec-container">
  ...
  <div class="exec-step exec-step-blue">
    <div class="exec-step-header">
      ...
      <div class="exec-step-title">Agent calls deer CLI</div>
    </div>
    <div class="exec-step-body">
      <div class="exec-cmd">
        <span class="exec-prompt">$</span> deer run-source my-golden-vm "systemctl status nginx"
      </div>
    </div>
  </div>
  ...
  <div class="exec-step exec-step-blue">
    ...
    <div class="exec-pass exec-pass-blue">
      <span class="exec-check exec-check-blue">v</span> Passes
    </div>
  </div>
  ...
  <div class="exec-step exec-step-purple">
    ...
    <ul class="exec-step-details">
      <li>Check cache for "sourcevm:my-golden-vm:deer-readonly"</li>
      <li>
        If expired: generate Ed25519 keypair, sign certificate with principal "deer-readonly" and
        30-min TTL
      </li>
      <li>Write to ~/.deer/keys/sourcevm-my-golden-vm/</li>
    </ul>
  </div>
  ...
  <div class="exec-step exec-step-orange">
    ...
    <ul class="exec-step-details">
      <li>Connect to VM IP as user "deer-readonly"</li>
      ...
      <li>sshd invokes login shell: deer-readonly-shell</li>
    </ul>
  </div>
  <div class="exec-conn exec-conn-orange"></div>
  <div class="exec-step exec-step-orange">
    ...
    <div class="exec-pass exec-pass-orange">
      <span class="exec-check exec-check-orange">v</span> Passes
    </div>
  </div>
...
Running `deer source prepare <vm-name>` configures the VM once. The preparation is idempotent -- running it again safely updates the restricted shell and CA key without breaking anything. Each step checks for existing state:

```go
// Create user only if it doesn't already exist
userCmd := `id deer-readonly >/dev/null 2>&1 || useradd -r -s /usr/local/bin/deer-readonly-shell -d /nonexistent -M deer-readonly`
...
The preparation state is tracked in SQLite so deer knows which VMs are ready:
...
If the CA key changes, the fingerprint mismatch tells deer the VM needs re-preparation.
...
<BlogInlineCTA feature="This is how Fluid lets agents inspect golden images without cloning - try it on your own servers." />
...
| Attacker bypasses restricted shell      | The `deer-readonly` user has no sudo access, no real home directory, and only standard non-privileged Unix write permissions (cannot modify system config or service data without escalation) |
...
| Credential stolen                       | 30-minute TTL limits window; certificate only grants `deer-readonly` principal                                                                                                                |
