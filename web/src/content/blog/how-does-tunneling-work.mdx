...
Back in high-school, my school system blocked all traffic going to popular social media apps like Snapchat, Youtube, and Instagram. The way that we all got around it was with using a free VPN to mask our traffic.

At the time, it felt like magic. You flip the VPN on, and suddenly YouTube loads. Flip it off, blocked again. But what actually happens when you hit "Connect"? Where does the traffic go, and why can't the school's firewall tell that you're watching videos instead of doing homework?

Turns out, it's not magic. It's tunneling. And once you understand how packets work, the whole thing clicks.
...
Your original packet says "send this to youtube.com." The firewall would block that. But if you wrap that packet inside a new packet that says "send this to 198.51.100.1" (your VPN server's IP), the firewall has no idea what's inside. It sees an allowed destination, and forwards it along.
...
The key insight is step 5: the outer IP header is the only thing visible to the firewall. Everything inside it - your real destination, your HTTP request, the actual data - is encrypted and opaque.
...
**OpenVPN** - the battle-tested workhorse. Can run over TCP or UDP, highly configurable, supports certificate-based auth. More overhead than WireGuard, but its TCP mode can masquerade as regular HTTPS traffic on port 443, making it harder to block in restrictive networks.
...
If you want to see how sandboxes themselves work under the hood, check out [How Fluid.sh Sandboxes Work](/blog/how-sandboxes-work).
