...
func TestSourceHostConns_Defaults(t *testing.T) {
	...
	if c.SshUser != "fluid-daemon" {
		t.Errorf("user: got %q, want fluid-daemon", c.SshUser)
	}
	if c.SshPort != 22 {
...
func TestSourceHostConns_CustomValues(t *testing.T) {
	...
	if conns[1].SshUser != "fluid-daemon" {
		t.Errorf("host1 user: got %q, want fluid-daemon", conns[1].SshUser)
	}
...
func TestResolveSourceHost_CacheHit(t *testing.T) {
	expected := &fluidv1.SourceHostConnection{
		Type: "libvirt", SshHost: "10.0.0.1", SshPort: 22, SshUser: "fluid-daemon",
	}
	s := &Server{
		cfg:         &config.Config{},
		vmHostCache: map[string]*fluidv1.SourceHostConnection{"my-vm": expected},
	}
