...
func TestScanSourceHostKeys_NoSourceHosts(t *testing.T) {
	s := newTestServer(&config.Config{})
	resp, err := s.ScanSourceHostKeys(context.Background(), &fluidv1.ScanSourceHostKeysRequest{})
	if err != nil {
...
func TestDoctorCheck_FullRPC(t *testing.T) {
	...
	resp, err := s.DoctorCheck(context.Background(), &fluidv1.DoctorCheckRequest{})
	if err != nil {
